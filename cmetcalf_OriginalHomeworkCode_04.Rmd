...
```{r}
model <- function(d) {
  lm(MaxLongevity_m ~ Brain_Size_Species_Mean, data=d)
}
model(d) #finds linear regression line of best fit, as well as the intercep tbeing beta0 and the remaining coefficient is beta1
```
```{r}
g <- ggplot(data = d, aes(x = MaxLongevity_m, y = Brain_Size_Species_Mean))
g <- g + geom_point()
g <- g + geom_smooth(method = "lm", formula = y ~ x)
g <- g + geom_text(data = d, aes(label = "model(d)", stat = "equation", position = "left", parse = TRUE))
...
```{r}
ci <- confint(model(d), level = 0.90)  # using the results of lm()
ci
```

```{r}
model2 <- lm(data = d, MaxLongevity_m ~ Brain_Size_Species_Mean)
h_hat <- predict(model2, newdata = data.frame(weight = d$Brain_Size_Species_Mean))
df <- data.frame(cbind(d$Brain_Size_Species_Mean, d$MaxLongevity_m, h_hat))
names(df) <- c("x", "y", "yhat")
head(df)
```
This was me messing with the predict example from later in the module, but it keeps giving me error messages :( 
